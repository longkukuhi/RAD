...
def save_eval_visualization(imgs, recon, anomaly_map, gt, save_path,
    ...
    amap = anomaly_map.clone()
    amap = amap - amap.amin(dim=(2, 3), keepdim=True)
    #amap = amap / (amap.amax(dim=(2, 3), keepdim=True) + 1e-8)
    max_vals = torch.empty(B, 1, 1, 1)
    for i in range(B):
        max_vals[i] = torch.quantile(amap[i], 0.98)
    ...
    gt_vis = gt.float()
    if gt_vis.shape[1] > 1:
        gt_vis = gt_vis.max(dim=1, keepdim=True)[0]

    B_show = B
    fig, ax = plt.subplots(B_show, 4, figsize=(14, 4 * B_show))
    ...
    for i in range(B_show):
        ...
        ax[i, 1].imshow(TF.to_pil_image(recon_vis[i]))
        ax[i, 1].set_title("Reconstruction")
        ax[i, 1].axis("off")

        ax[i, 2].imshow(amap[i, 0].numpy(), cmap='jet', vmin=0, vmax=1) 
        ax[i, 2].set_title("Anomaly Map")
        ax[i, 2].axis("off")

        ax[i, 3].imshow(TF.to_pil_image(gt_vis[i]))
        ax[i, 3].set_title("GT mask")
        ax[i, 3].axis("off")
...
def save_eval_visualization_masked(
    imgs,
    recon,
    mae_masks,
    gt,
    ...
    imgs:      [B, 3, H, W]   
    recon:     [B, 3, H, W]   
    mae_masks: [B, L]  - MAE patch masks        
    gt:        [B, 1 or C, H_gt, W_gt] - GT mask
    """

    imgs = imgs.detach().cpu()
    recon = recon.detach().cpu()
    mae_masks = mae_masks.detach().cpu()
    gt = gt.detach().cpu()
    ...
    # ---------- MAE patch mask -> image ----------
    L = mae_masks.shape[1]
    h = w = int(L ** 0.5)  
    masks = mae_masks.view(B, 1, h, w).float()   # [B,1,h,w]
    ...
    if gt.ndim == 4:
        if gt.shape[1] > 1:
            gt_vis = gt.float().max(dim=1, keepdim=True)[0]
        else:
            gt_vis = gt.float()
    else:  # [B, H, W]
        gt_vis = gt.float().unsqueeze(1)

    if gt_vis.shape[-2:] != (H, W):
        gt_vis = F.interpolate(gt_vis, size=(H, W), mode="nearest")

    gt_vis = gt_vis.clamp(0, 1)

    B_show = min(4, B)
    fig, ax = plt.subplots(B_show, 4, figsize=(16, 4 * B_show))
    ...
    for i in range(B_show):
        ...
        ax[i, 1].set_title("Masked Original")
        ax[i, 1].axis("off")
...
def evaluation_batch_mae(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None, vis_dir=None, max_vis=8):
    model.eval()
    gt_list_px = []
    pr_list_px = []
    gt_list_sp = []
    pr_list_sp = []
    # get_gaussian_kernel is assumed to return a callable (e.g., nn.Module)
    gaussian_kernel = get_gaussian_kernel(kernel_size=25, sigma=6).to(device)
    ...
    with torch.no_grad():
        for img, gt, label, img_path in dataloader:
            ...
            gt = gt.to(device)        # [B,1(or C),H,W]
            label = label.to(device)

            # starter.record()
            recon = model(img, is_training=False, full_cov_eval=True)  # [B,3,H,W]
            # ender.record()
            # torch.cuda.synchronize()
            # curr_time = starter.elapsed_time(ender)

            # ---- 1. Pixel-level reconstruction error -> anomaly map ----
            # (Can try L1 / MSE / normalization, etc., using L1 here)
            #anomaly_map = (img - recon) ** 2       # [B,3,H,W]
            #anomaly_map = anomaly_map.mean(dim=1, keepdim=True)  # [B,1,H,W]
            diff = torch.abs(img - recon) # [B, 3, H, W]
            anomaly_map = diff.mean(dim=1, keepdim=True) # [B, 1, H, W]
            anomaly_map = torch.clamp(anomaly_map, min=0, max=0.5)
            anomaly_map = anomaly_map * 2.0  # [B,1,H,W]

            # ---- 2. Resize to match mask dimensions ----
            ...
            # ---- 4. Process gt (binary, single-channel) ----
            gt = gt.bool()
            if gt.shape[1] > 1:
                # Collapse multi-channel mask to single-channel
                gt = torch.max(gt, dim=1, keepdim=True)[0]

            if (vis_dir is not None) and (vis_count < max_vis):
                remain = max_vis - vis_count
                b_vis = min(img.shape[0], remain)
                save_path = os.path.join(
                    vis_dir,
                    f"vis_{vis_count:03d}.png" if _class_ is None
                    else f"{_class_}_vis_{vis_count:03d}.png"
                )
            ...
            # ---- 5. Image-level score: max or top-k mean ----
            if max_ratio == 0:
                # Use max pixel value as the image-level score
                sp_score = torch.max(anomaly_map.flatten(1), dim=1)[0]
            else:
                # Use the mean of top-k% pixel values
                amap_flat = anomaly_map.flatten(1)
                k = int(amap_flat.shape[1] * max_ratio)
                topk_vals = torch.sort(amap_flat, dim=1, descending=True)[0][:, :k]
                sp_score = topk_vals.mean(dim=1)

            pr_list_sp.append(sp_score)

        # ---- 6. Concatenate into numpy arrays for metric calculation ----
        ...
        # AUPRO (Calculated before flattening pixels)
        aupro_px = compute_pro(gt_list_px, pr_list_px)

        # Flatten pixels for standard pixel-wise metrics
        ...
        # Assumes f1_score_max finds the best threshold
